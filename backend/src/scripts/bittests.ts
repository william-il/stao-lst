...
async function main() {
  ...
  keyring.getPairs().forEach((pair) => {
    if (pair.meta.name !== undefined && typeof pair.meta.name === 'string') {
...
async function getTransferFee(
  ...
  const transferTx = api.tx.balances.transferAllowDeath(recipient.address, 1n);
    ...
    for (let i = 0; i < numberOfTransactions; i++) {
      try {
        ...
        const tx = await api.tx.balances
          .transferAllowDeath(
            recipient.address,
            Array.isArray(amountToSend) ? amountToSend[i] : amountToSend
          )
  ...
  const unsubscribe = await api.rpc.chain.subscribeNewHeads(async (header) => {
    ...
    if (header.number.eq(finalizedHeader.number)) {
      ...
      if (currentBalance !== lastKnownBalance) {
        ...
        console.log(`${keyPair.meta.name} balance: ${currentBalance}`);
        lastKnownBalance = currentBalance;
      }
    }
  });
...
async function blockSubscription(
  ...
  const blockHash = await api.rpc.chain.subscribeFinalizedHeads(
    async (finalizedHeader) => {
      ...
      const signedBlock = await api.rpc.chain.getBlock(finalizedHeader.hash);
