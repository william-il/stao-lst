...
export default class EthersTestUtils {
    ...
    public async relativeUpdateRatiosWithCycle(
        ...
        for (let i = 0; i < _cycles; i++) {
            let tx = await this.updateRatiosWithBasisPoints(
                (await this.poolTaxContract.redeemBasisPoints()) +
                    _cycleIncrement
            );
            receiptArr.push(tx);
        }
    ...
    private emitTransactionReciptEvent(
        ...
        const eventLogsArr = receipt?.logs.filter(
        ...
        if (eventLogsArr !== undefined && eventLogsArr.length > 0) {
            eventLogsArr.forEach((eventLog) => {
                let nameArr = eventLog.fragment.inputs.map(
                    (input) => input.name
                );
    ...
    public async queryEventsInRangeAndBatch(
        ...
        if (eventName === 'Redeemed' || eventName === 'Staked') {
            const namesArray = lastEvents[0].fragment.inputs.map(
                (name) => name.name
            );
    ...
    public async addEthKeyAccount(
        ...
        try {
            ...
            tx = await tempContract.addAccount(
                taoAddress,
                ethWallet,
                signature
            );
    ...
    public async retrieveEthKeyAccount(taoAddress: string): Promise<EthKey[]> {
        try {
            const result =
                await this.contract.getAllEthKeysForTaoAddress(taoAddress);
            const allEthKeys: EthKey[] = result.map(
                (item: [string, string]) => ({
                    address: item[0],
                    signature: item[1],
                })
