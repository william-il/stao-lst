...
export default class BittensorTestUtils {
                ...
                .signAndSend(sender, { nonce }, ({ status, events }) => {
                    if (status.isInBlock) {
                        ...
                        const extrinsicFailed = events.find(({ event }) => {
                            return this.api.events.system.ExtrinsicFailed.is(
                                event
                            );
                        });
                        if (extrinsicFailed) {
    ...
    public async getStakingInterval() {
        ...
        console.log(
            (
                await this.api.query.subtensorModule.targetStakesPerInterval()
            ).toHuman()
        );
    }
    ...
    */
        ...
        const unsubscribe = await this.api.rpc.chain.subscribeFinalizedHeads(
            async (finalHeader) => {
                ...
                if (currentBalance !== lastKnownBalance) {
                    ...
                    console.log(
                        `${keyPair.meta.name} balance2: ${currentBalance}`
                    );
                    ...
                    try {
                        ...
                        const events = allRecords
                            ...
                            .forEach(({ event }) => {
                                ...
                                if (
                                    this.api.events.system.ExtrinsicSuccess.is(
                                        event
                                    ) &&
                                    ...
                                    this.api.events.system.ExtrinsicFailed.is(
                                        event
                                    )
                                ) {
    ...
    public convertToCodeReadableBigInt(
        ...
        const isValid = /^-?\d{1,3}(,\d{3})*(\.\d+)?$/.test(
            humanReadableNumber
        );
        if (!isValid) {
        ...
        const blockHash = await this.api.rpc.chain.subscribeFinalizedHeads(
            async (finalizedHeader) => {
                if (shouldLog) {
                    ...
                    console.log(
                        'block number: ',
                        finalizedHeader.toHuman().number
                    );
                    console.log('block hash: ', finalizedHeader.hash.toHuman());
                }
                                    ...
                                    if (
                                        ...
                                        if (
                                            ...
                                            const amountAsNumber =
                                                this.convertToCodeReadableBigInt(
                                                    event.toHuman().data.amount
                                                );
                                            if (amountAsNumber !== null) {
                                                const obj = {
                                                    from: event.toHuman().data
                                                        .from,
